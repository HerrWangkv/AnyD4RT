...
6. **第二次运行**：用组合奖励，帧间隔 2 和 1 各 30 段，结果如下。
   - 命令：`CUDA_VISIBLE_DEVICES=<gpu> .venv/bin/python scripts/s1_checks.py --stride {2,1} --out outputs/s1_comb_stride{2,1}`，每段约 11 秒。
...
- **40 帧接口成立（§3.4）**：
  - 在共同时间点上，40 帧输入与 48 帧输入的奖励几乎相同：均值差 < 0.002，单段最大差 0.015；
  - 逐帧深度 AbsRel：48 帧为 0.060 / 0.058，40 帧为 0.063 / 0.060（斜线两边分别是帧间隔 2 和 1）。
- **尺度拟合**：60 段全部有效。
...
| A2 | **物体级退化测试**：平移、删除、冻结物体区域，整段模糊。PointOdyssey 有逐帧实例掩码（`masks/`）。只用于诊断，不需要编辑后的 GT（§5.1(a)）。 | 每种退化的泄漏率，即退化后奖励 ≥ 原视频奖励的比例。 |
...
3. ~~40 帧接口（S1）~~：S1 已验证成立；40 帧和 48 帧 clip 分开组成 microbatch（§3.4）。
