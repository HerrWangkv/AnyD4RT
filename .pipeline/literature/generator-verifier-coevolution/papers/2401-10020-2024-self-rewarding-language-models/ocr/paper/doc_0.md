...
GenerateresponsesGeneraterewardsPreference pairsDPO training selectGenerated new promptsSelf-Instruction creationInstruction following trainingNext iteration modelSeed model(for t=1)2022]. We believe this can increase the ceiling of the potential for self-improvement of these
...
AI Feedback Training After performing the self-instruction creation procedure, we
...
Iterative Training Our overall procedure trains a series of models M1, . . . , MT where
...
1,630 train and 541 evaluation examples (which do not overlap with the IFT data).
...
Instruction Following We evaluate head-to-head performance between various models
...
Reward Modeling We evaluate the correlation with human rankings on the evaluation set
...
Instruction following training The training hyperparameters we use are as follows. For
...
Self-Instruction creation To generate new prompts we use a fixed model, Llama 2-Chat
...
EFT+IFT seed training performs similarly to IFT alone We find that adding
...
Iteration 2 (M2) improves over Iteration 1 (M1) and SFT Baseline Iteration 2
...
Iteration 3 (M3) improves over Iteration 2 (M2) We see a further gain in Iteration 3
...
Self-Rewarding models perform well on AlpacaEval 2 leaderboard We evaluate
...
Fine-grained analysis As described earlier, the overall performance of the model in
...
Self-Rewarding M3vs.SFT BaselineSelf-Rewarding M2vs.SFT BaselineSelf-Rewarding M1vs.SFT Baseline62.549.230.527.736.338.79.814.530.9Self-Rewarding WinsTieSFT Baseline WinsSelf-Rewarding M3vs.M2Self-Rewarding M2vs.M1Self-Rewarding M3vs.M147.755.568.839.832.822.712.511.78.6Left Wins (in Left vs. Right)TieRight WinsTable 1: AlpacaEval 2.0 results (win rate over GPT-4 Turbo evaluated by GPT-4).
...
Win Rate Distilled Proprietary
...
Data distribution analysis We perform a t-SNE [Van der Maaten and Hinton, 2008]
...
HealthProfessionalLinguisticsOtherEntertainmentTechnologyLiteratureCodingScienceGamingPhilosophySocialStudiesTravelArtsSportsMathematicsSocialInteractionDIYProjectsCooking05101520253035Winrate(%)M0M1M2M3Iterations of Self-Rewarding (M1, M2 and
...
Human evaluation To examine whether human judgments align with automatic evaluation
...
MT-Bench performance further validates these results We report performance on
MT-Bench in Table 2 for the SFT baseline and iterations of the Self-Rewarding model. We

9

Self-Rewarding M3vs.SFT BaselineSelf-Rewarding M2vs.SFT BaselineSelf-Rewarding M1vs.SFT Baseline66.056.028.016.024.026.018.020.046.0Self-Rewarding WinsTieSFT Baseline WinsTable 4: Reward Modeling ability improves with Self-Training: We evaluate the LLM-
...
Self-rewarding models did not lose ability on NLP Benchmarks As shown in
...
EFT augmentation improves over SFT baseline Firstly, we find that adding Eval-
...
Reward Modeling ability improves with Self-Training We find that performing
...
Reinforcement Learning from Human Feedback (RLHF) Preference learning ap-
...
Reinforcement Learning from AI Feedback (RLAIF) Constitutional AI [Bai et al.,
...
Improving LLMs via data augmentation (and curation) Several methods have
...
LLM-as-a-Judge Using LLM-as-a-Judge prompting to evaluate language models has
...
Other EFT prompts we have tried At first, we took the EFT prompt from Li et al.
...
10050050Dimension 17550250255075Dimension 2IFT dataEFT dataAIFT data50050Dimension 1806040200204060Dimension 2IFT dataEFT dataAIFT dataBelow is a question from an user and a candidate response. Please grade the
...
Secondly, score the instruction in terms of complexity: how complex you think it
...
Thirdly,
indicate how long you think the response to the instruction should be, ei-
...
Self-Rewarding M03vs.SFT BaselineSelf-Rewarding M02vs.SFT Baseline50.446.532.834.816.818.8Self-Rewarding WinsTieSFT Baseline WinsSelf-Rewarding M3vs.M03Self-Rewarding M2vs.M0238.734.844.536.716.828.5Left Wins (in Left vs. Right)TieRight WinsTable 6: Breakdown of AlpacaEval test set instructions by instruction category.
Number Percentage
Category
...
Table 7: Breakdown of AlpacaEval test set instructions by instruction complexity. The
...
12345678InstructionComplexity510152025Winrate(%)M0M1M2M31sentence1-3sentences1paragraph2paragraphsExpectedresponselength510152025Winrate(%)M0M1M2M3Table 8: Breakdown of AlpacaEval test set instructions by expected response length.
...
A.4 Preference optimization outperforms augmenting with positive examples
...
in all 30 cases. However, for M 2 and M 3, the model is likely to first generate a few
